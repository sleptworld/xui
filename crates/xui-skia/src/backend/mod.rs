...
use self::{
    convert::valid_scale,
    image::{image_cache, source_image_cache, CachedImageKey, CachedSourceImage},
    lru::LocalLru,
    paint::GradientKey,
    surface::{copy_surface_damage, full_softbuffer_rect, physical_damage_rects, SurfacePool},
    text::CachedTextBlob,
...
impl<T: TextBackend> SkiaBackend<T> {
    pub fn new(window: Arc<dyn SurfaceTarget>) -> Result<Self, SkiaBackendError> {
    ...
    /// [`xui_core::widgets::CanvasPainter::texture`].
    ///
