...
pub(crate) struct SoftwarePresenter {
    _window: Arc<dyn SurfaceTarget>,
    _context: Context<Arc<dyn SurfaceTarget>>,
    pub(crate) surface: SoftSurface<Arc<dyn SurfaceTarget>, Arc<dyn SurfaceTarget>>,
}

impl SoftwarePresenter {
    fn new(window: Arc<dyn SurfaceTarget>) -> Result<Self, SkiaBackendError> {
...
impl WindowPresenter {
    ...
    pub(crate) fn new(window: Arc<dyn SurfaceTarget>) -> Result<PresenterSetup, SkiaBackendError> {
        if gpu_disabled() {
    ...
    #[cfg(feature = "wgpu")]
    fn new_shared(window: Arc<dyn SurfaceTarget>) -> Result<PresenterSetup, SkiaBackendError> {
    ...
    #[cfg(target_os = "macos")]
    fn new_gpu(window: Arc<dyn SurfaceTarget>) -> Result<(Self, DirectContext), SkiaBackendError> {
    ...
    fn new_gpu(_window: Arc<dyn SurfaceTarget>) -> Result<(Self, DirectContext), SkiaBackendError> {
