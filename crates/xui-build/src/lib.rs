...
//! `cargo::rustc-env`, which is where `xui_core::include_assets!()` looks. Plain
...
pub fn generate(
    ...
    let runtime = runtime_crate(package_dir);
    let build_config = BuildConfig {
        package: PackageConfig {
            ...
            asset_id_path: format!("{runtime}::assets::AssetId"),
        },
    ...
    let source = bootstrap_source(&layout, config, fallbacks, &runtime)?;
...
/// The crate generated code names the runtime by: `xui-core` when the package
/// depends on it directly, otherwise the `xui` facade that re-exports it, under
/// whatever name the manifest gives either. Mirrors `krate::xui` in `xui-macros`.
fn runtime_crate(package_dir: &Path) -> String {
    let manifest = fs::read_to_string(package_dir.join("Cargo.toml"))
        .ok()
        .and_then(|text| text.parse::<toml::Table>().ok());
    let Some(manifest) = manifest else {
        return "xui_core".into();
    };
    ...
    if let Some(targets) = manifest.get("target").and_then(toml::Value::as_table) {
        tables.extend(
            targets
                .values()
                .filter_map(|target| target.get("dependencies")?.as_table()),
        );
    }
        ...
        tables
            ...
            .find_map(|(key, value)| {
                let name = value
                    ...
                    .unwrap_or(key);
                (name == package).then(|| key.replace('-', "_"))
            })
    };
    find("xui-core")
        .or_else(|| find("xui"))
        .unwrap_or_else(|| "xui_core".into())
}

/// The module `xui_core::include_assets!()` includes.
fn bootstrap_source(
    layout: &Layout,
    config: &AssetsConfig,
    fallbacks: Fallbacks,
    ...
    body.push_str(&format!(
        "        {runtime}::assets::bootstrap::mount_overlay(&mut manager, dev_directory);\n",
    ));
    match config.bundle {
        BundleMode::Embedded => body.push_str(&format!(
            "        manager.mount({runtime}::assets::EmbeddedPak::new(include_bytes!({}))?);\n",
            rust_path_literal(&layout.pak)?
        )),
        BundleMode::External => {
            body.push_str(&debug_binding("fallback_pak", Some(&layout.pak))?);
            body.push_str(&format!(
                "        manager.mount({runtime}::assets::bootstrap::external_pak({:?}, fallback_pak)?);\n",
                config.output
            ));
        }
    }
    Ok(format!(
        "// @generated by xui-build. Do not edit.\n\
         ...
         \x20   pub fn manager() -> Result<{runtime}::assets::AssetManager, {runtime}::assets::AssetError> {{\n\
         \x20       let mut manager = {runtime}::assets::AssetManager::new();\n\
         {body}\
         \x20       Ok(manager)\n\
         \x20   }}\n\
         }}\n"
