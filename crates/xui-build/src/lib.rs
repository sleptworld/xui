...
use thiserror::Error;
use xui_pak_build::{build_empty_to, build_to, BuildConfig, PackageConfig};
...
pub fn generate(
    ...
    let build_config = BuildConfig {
        package: PackageConfig {
            ...
            asset_id_path: "xui_core::assets::AssetId".into(),
        },
...
fn bootstrap_source(
    ...
    body.push_str(
        "        xui_core::assets::bootstrap::mount_overlay(&mut manager, dev_directory);\n",
    );
    match config.bundle {
        BundleMode::Embedded => body.push_str(&format!(
            "        manager.mount(xui_core::assets::EmbeddedPak::new(include_bytes!({}))?);\n",
            rust_path_literal(&layout.pak)?
        )),
        BundleMode::External => {
            body.push_str(&debug_binding("fallback_pak", Some(&layout.pak))?);
            body.push_str(&format!(
                "        manager.mount(xui_core::assets::bootstrap::external_pak({:?}, fallback_pak)?);\n",
                config.output
            ));
        }
    }
    Ok(format!(
        "// @generated by xui-build. Do not edit.\n\
         ...
         \x20   pub fn manager() -> Result<xui_core::assets::AssetManager, xui_core::assets::AssetError> {{\n\
         \x20       let mut manager = xui_core::assets::AssetManager::new();\n\
         {body}\
         \x20       Ok(manager)\n\
         \x20   }}\n\
         }}\n"
...
mod tests {
    ...
    fn assets_are_packed_and_named() {
        ...
        assert!(fs::read_to_string(&layout.refs)
            .unwrap()
