...
impl<T: TextBackend> WGPUBackend<T> {
    pub fn new(window: Arc<dyn SurfaceTarget>) -> Result<Self, WgpuBackendInitError> {
        Self::new_with_options(window, WgpuBackendOptions::default())
    }

    pub fn new_with_options(
        window: Arc<dyn SurfaceTarget>,
        options: WgpuBackendOptions,
...
struct LayerGraphDrawRecord {
    ...
    program: xui_core::render::render_graph::BuiltLayerProgram,
...
struct PrefixStageDemand {
    prefix: xui_core::render::SurfacePrefix,
    demand: Rect,
    placement: Option<xui_core::render::BuiltLayerInstanceId>,
}
...
impl<T: TextBackend> WGPUBackend<T> {
    ...
    fn apply_backdrop_only_to_prefix(
        ...
        encoder: &mut wgpu::CommandEncoder,
        frame: &BuiltFrame,
        instance_id: xui_core::render::BuiltLayerInstanceId,
