...
mod tests {
    ...
    #[test]
    fn pbutton_forwards_click_handler() {
        let clicked = Rc::new(Cell::new(0));
        let clicked_for_handler = clicked.clone();
        let mut app = App::with_component_registry(|registry| {
            register_components(registry);
            move |_| {
                Element::from(pbutton(ButtonProps::new("Save").on_click({
                    let clicked_for_handler = clicked_for_handler.clone();
                    move |_| {
        ...
        let mut measurer = xui::font::TextI::new();
        let mut backend = MockRenderBackend::default();

        app.resize(Size::new(200.0, 80.0));
    ...
    #[test]
    fn disabled_pbutton_does_not_attach_click_handler() {
        let clicked = Rc::new(Cell::new(0));
        let clicked_for_handler = clicked.clone();
        let mut app = App::with_component_registry(|registry| {
            register_components(registry);
            move |_| {
                Element::from(pbutton(ButtonProps::new("Save").disabled(true).on_click({
                    let clicked_for_handler = clicked_for_handler.clone();
                    move |_| {
                        clicked_for_handler.set(clicked_for_handler.get() + 1);
                        EventResult::Consumed
    ...
    #[test]
    fn pbutton_accepts_jsx_children_from_elements_and_expressions() {
        let mut app = App::with_component_registry(|registry| {
            register_components(registry);
            |_| {
                xui! {
                    <pbutton props={ButtonProps::new("fallback")
                        .variant(ButtonVariant::Secondary)
                        .size(ButtonSize::Lg)}>
                        <text>{"child"}</text>
                        {Element::from(text("expr"))}
                    </pbutton>
                }
            }
        });
        ...
        app.resize(Size::new(240.0, 100.0));
        app.render(&mut backend, &mut measurer).unwrap();

        let root = app.arena().root();
        let button_id = app.arena().children(root)[0];
        assert_eq!(app.arena().children(button_id).len(), 2);
        assert_eq!(
            app.arena().node(button_id).unwrap().node_type,
            xui::widgets::WidgetType::Button
        );
    }
}
