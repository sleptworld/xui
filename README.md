...
#[xui_core::main]
fn main() {
    xui_core::include_assets!(); // generated by the build script
...
script hands `xui_core::include_assets!()` its generated module through
...
```text
 ...
 (swash stack)  (cosmic-text impl)  (layer/filter IR)    (Skia backend)
   ...
   └──────────────────┴───────────────────┴───────────────────┘
                                   │
                  xui-shell (host contract) ◄── xui-winit (window/event loop)
                                   │
...
2. `xui-winit` translates them into `xui_shell::ShellEvent`s, and
   `xui_shell::Shell` turns those into `xui_interface::events::RawEvent`s.
